...
#define MAXTOKEN 25
typedef enum { T_NONE, T_IDENT, T_NUM, T_ADD, T_MUL, T_SUB, T_POW,
	       T_LBRA, T_RBRA } TokenId;
...
void get_oper(struct Scanner_Context *scon)
{
	...
	case '^' : scon->t.id = T_POW; break;
	case '(' : scon->t.id = T_LBRA; break;
	case ')' : scon->t.id = T_RBRA; break;
